...
for barcode in barcodes:
    ...
    # Extract polygon pts
    pts = barcode.polygon
    pts = np.array([(point.x, point.y) for point in pts], dtype=np.int32)

    # Expand the bounding box size
    scale_factor = 1.2  # Increase rectangle size by 20%
    centroid = np.mean(pts, axis=0)  # Compute the center of the polygon
    expanded_pts = np.array([centroid + scale_factor * (point - centroid) for point in pts], dtype=np.int32)

    # Draw an expanded rectangle around the barcode
    cv2.polylines(image, [expanded_pts], True, (0, 255, 0), 3)  # Thicker green rectangle

    # Annotate the decoded data beside the bounding box
    x, y = expanded_pts[0]  # Use the first point of the bounding box
...
out_file = "decoded_barcode_expanded.png"
